...
for dataset in dataset_names:
    ...
    for added in addeds:
        if os.path.exists(f"/global/cfs/cdirs/des/shubh/transformers/ViT_weak_lensing/2par/temp/RN_{dataset}{added}.lock") or \
          os.path.exists(f"/global/cfs/cdirs/des/shubh/transformers/ViT_weak_lensing/2par/temp/RN_{dataset}{added}.done") or \
          not os.path.exists(f"{cache_dir}/20230814_224x224/{dataset}{added}/1.1.0/{hashcode}/") or \
        ...
        if f"{dataset}{added}" in done:
            ...
            os.rmdir(f"/global/cfs/cdirs/des/shubh/transformers/ViT_weak_lensing/2par/temp/RN_{dataset}{added}.lock")
        else:
            Path(f"/global/cfs/cdirs/des/shubh/transformers/ViT_weak_lensing/2par/temp/RN_{dataset}{added}.lock").mkdir(parents=True, exist_ok=True)
            print(f"Running ResNet on {dataset}{added}")
            os.system(f"cd /global/cfs/cdirs/des/shubh/transformers/ViT_weak_lensing/; conda activate vit; python resnet.py {dataset}{added} 0;")
            # Path(f"/global/cfs/cdirs/des/shubh/transformers/ViT_weak_lensing/2par/temp/RN_{dataset}{added}.done").mkdir(parents=True, exist_ok=True)
            os.rmdir(f"/global/cfs/cdirs/des/shubh/transformers/ViT_weak_lensing/2par/temp/RN_{dataset}{added}.lock")
        # break