...
for dataset in dataset_names:
    ...
    for added in addeds:
        if os.path.exists(f"/global/cfs/cdirs/des/shubh/transformers/ViT_weak_lensing/new/temp/{dataset}{added}.lock") or \
          os.path.exists(f"/global/cfs/cdirs/des/shubh/transformers/ViT_weak_lensing/new/temp/{dataset}{added}.done") or \
        ...
        if f"{dataset}{added}" in done:
            ...
            if f"{dataset}{added}" in preds_done:
                continue
            Path(f"/global/cfs/cdirs/des/shubh/transformers/ViT_weak_lensing/new/temp/{dataset}{added}.lock").mkdir(parents=True, exist_ok=True)
            os.system(f"cd /global/cfs/cdirs/des/shubh/transformers/ViT_weak_lensing/; conda activate vit; python ViT.py {dataset}{added} 1;")
            os.rmdir(f"/global/cfs/cdirs/des/shubh/transformers/ViT_weak_lensing/new/temp/{dataset}{added}.lock")
            break
        else:
            Path(f"/global/cfs/cdirs/des/shubh/transformers/ViT_weak_lensing/new/temp/{dataset}{added}.lock").mkdir(parents=True, exist_ok=True)
            ...
            # Path(f"/global/cfs/cdirs/des/shubh/transformers/ViT_weak_lensing/new/temp/{dataset}{added}.done").mkdir(parents=True, exist_ok=True)
            os.rmdir(f"/global/cfs/cdirs/des/shubh/transformers/ViT_weak_lensing/new/temp/{dataset}{added}.lock")
        # break